...
@Component({
  selector: 'app-elasticity_index',
  templateUrl: './elasticity_index.component.html',
  styleUrls: ['./elasticity_index.component.css']
})


export class ElasticityIndexComponent extends BaseTableComponent implements OnInit, OnDestroy {
  title = "Order flow";
  btnSave = false;
  ...
  @ViewChild("elasticityComponent") elasticityComponent: ElasticityComponent;
  @ViewChild("elasticityWeekComponent") elasticityWeekComponent: ElasticityWeekComponent;
 ...
 */
  ...
  async getData(): Promise<Product[]> {
    ...
    const cached = sessionStorage.getItem('nn');
    if (cached && false) {
      this.dbd_stopBtn = false;
      this.loading = false;
      this.orders_all = JSON.parse(cached);
    }
    else {
      ...
      // else
      //   this.orders_all = await this.awsService.getAllOrders(this.start, this.stop);

      this.orders_all = await this.awsService.getAllOrdersV2(this.start, this.stop);
      // sessionStorage.setItem('nn', JSON.stringify(this.orders_all));
    }
